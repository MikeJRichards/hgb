...
shared ({ caller = _owner }) actor class Ledger() = this {
  ...
  func makeGenesisChain() : TxLog {
    validateSubaccount(HgbToken.init.minting_account.subaccount);

    let now = Nat64.fromNat(Int.abs(Time.now()));
    let log = Buffer.Buffer<Transaction>(100);
    for ({ account; amount } in Array.vals(HgbToken.init.initial_mints)) {
      validateSubaccount(account.subaccount);
      let tx : Transaction = {
        operation = #Mint({
          spender = HgbToken.init.minting_account;
          source = #Init;
          from = HgbToken.init.minting_account;
          to = account;
          amount = amount;
          fee = null;
          memo = null;
  ...
  func classifyTransfer(log : TxLog, transfer : Transfer) : Result<(Operation, Tokens), TransferError> {
    let minter = HgbToken.init.minting_account;
    ...
    let result = if (accountsEqual(transfer.from, minter)) {
      ...
      if (transfer.amount < HgbToken.init.transfer_fee) {
        return #Err(#BadBurn { min_burn_amount = HgbToken.init.transfer_fee });
      };
      ...
      let effectiveFee = HgbToken.init.transfer_fee;
      if (Option.get(transfer.fee, effectiveFee) != effectiveFee) {
        return #Err(#BadFee { expected_fee = HgbToken.init.transfer_fee });
      };
  ...
  public query func icrc1_minting_account() : async ?Account {
    ?HgbToken.init.minting_account;
  };

  public query func icrc1_name() : async Text {
    HgbToken.init.token_name;
  };

  public query func icrc1_symbol() : async Text {
    HgbToken.init.token_symbol;
  };

  public query func icrc1_decimals() : async Nat8 {
    HgbToken.init.decimals;
  };

  public query func icrc1_fee() : async Nat {
    HgbToken.init.transfer_fee;
  };

  public query func icrc1_metadata() : async [(Text, Value)] {
    [
      ("icrc1:name", #Text(HgbToken.init.token_name)),
      ("icrc1:symbol", #Text(HgbToken.init.token_symbol)),
      ("icrc1:decimals", #Nat(Nat8.toNat(HgbToken.init.decimals))),
      ("icrc1:fee", #Nat(HgbToken.init.transfer_fee)),
    ];
  };
  ...
  public shared ({ caller }) func icrc2_approve({
    ...
    if (approverBalance < HgbToken.init.transfer_fee) {
